...
`Agent Launcher` 会探测本机安装的 Agent CLI，让你选择要启动的 CLI，并在启动前选择本次会话启用哪些全局 skills / plugins。确认后，它会执行所选 CLI，并保留原本的命令行参数。
...
- 全局安装了很多 skills / plugins，希望按会话临时启停。
...
↑/↓ move/scroll, ←/→ collapse/expand, Space toggle selection, Enter continue, a toggle all, q cancel
```

用上下方向键移动，右方向键展开当前分组，左方向键折叠当前分组；在子项行按右方向键会忽略，在子项行按左方向键会折叠当前分组并把光标定位到分组行。空格切换当前行的选择状态：在子项行反选单项，在分组行切换整组。回车继续，`a` 全选/全不选，`q` 取消。超过 5 项的分组会默认折叠。selector 固定最多展示 20 行，超出的内容用方向键滚动查看。

skill 会根据已选择的 CLI 从对应目录读取：
...
相同前缀的同类项会分到同一组，例如 `lark-approval`、`lark-apps` 会进入 `lark` 分组；如果同时存在 `understand` 和 `understand-chat`，两者也会进入 `understand` 分组。skills 和 plugins 分别展示，并按名称排序。

### Codex 的禁用方式

选择 `codex` 时，反选的 skills / plugins 会转成启动参数：
